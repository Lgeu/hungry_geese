...
struct Min {
    ...
    constexpr Min() : data() {
        ...
        data[(int)Features::RELATIVE_POSITION_OPPONENT_TAIL] = 1;
...
struct Max {
    ...
    constexpr Max() : data() {
        ...
        data[(int)Features::RELATIVE_POSITION_OPPONENT_TAIL] = 76;
...
void ExtractFeatures(
    ...
    for (int idx_agents = 0; idx_agents < 4; idx_agents++) {
        ...
        for (int opponent = 0; opponent < 4; opponent++) {
            ...
            const auto& opponent_tail = geese[opponent].back();
            const auto relative_opponent_tail = CalcRelativePosition(head, opponent_tail);
            ASSERT_RANGE(relative_opponent_tail, MIN[Features::RELATIVE_POSITION_OPPONENT_TAIL], MAX[Features::RELATIVE_POSITION_OPPONENT_TAIL] + 1);
            features.push(OFFSET[Features::RELATIVE_POSITION_OPPONENT_TAIL] + relative_opponent_tail);
        }
