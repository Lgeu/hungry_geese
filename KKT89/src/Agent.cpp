...
void Agent::setActions(Stage& aStage) {
	// 4体毎の処理はここに書く
	for (int i = 0; i < 4; ++i) {
...
void Agent::setActions(Stage& aStage, int aIndex) {
	// 貪欲解
	GreedyAgent(aStage,aIndex);
}

void Agent::GreedyAgent(Stage& aStage, int aIndex) {
	...
	aStage.mActions[aIndex] = Idx_to_Actions[opt_action];
	return;
}

int Agent::min_Distance(Point aPos, Point bPos) {
	int result = 0;
...
Point Agent::Translate(Point aPos, int Direction) {
	int nx = aPos.x;
	int ny = aPos.y;
	nx += dx[Direction];
	if (nx < 0) {
